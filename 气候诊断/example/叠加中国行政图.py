...
prec = ds['pre'].loc[:,0:,-90:160]
lat = ds['lat'].loc[0:]
lon = ds['lon'].loc[-90:160]

uwnd = du['uwnd'].loc[:,700,:0,-90:160]
vwnd = dv['vwnd'].loc[:,700,:0,-90:160]
...
for myseason in ["SON","OND","NDJ","DJF"]:
	...
	fig, ax = draw_PlateCarree(10,60,70,140)
	...
	ax.add_feature(statesFeat, linewidth=0.5, edgecolor='k')
   ...
   # plot significant regions
	...
	#uvflux_not_sig = ax.quiver(ulon, ulat, u_reg_xr, v_reg_xr, transform=ccrs.PlateCarree(), pivot='mid', width=0.0028, scale=5.0, headwidth=4, color="gray")
	uvflux_sig = ax.streamplot(ulon, ulat, u_reg, v_reg, transform=ccrs.PlateCarree(), color="k", linewidth=1) #, pivot='mid', width=0.0028, scale=5.0, headwidth=4, color="k")
